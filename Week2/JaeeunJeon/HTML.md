...
### GET과 POST의 차이
* GET: **데이터를 요청**할 때 사용된다. 서버에서 정보를 가져와 클라이언트에 표시하거나 처리할 때 줄 사용한다.
  데이터는 URL 파라미터로 전달한다. 주로 데이터 조회에 사용되며, 보안이 필요한 정보 전달에는 적합하지 않다.
  - 예시: https://example.com/search?query=HTML
* POST: **데이터를 서버에 전송**하여 서버의 상태를 변경하거나 저장할 때 사용한다.
  데이터는 본문(body)에 담아 서버로 전송한다. 주로 데이터 입력 또는 변경 작업에 사용되며, 보안이 필요한 정보 전달에 적합하다.
  - 예시:
...
링크를 클릭할 때 바로 파일을 다운로드하도록 할 수 있다.
```
<a href="/files/resume.pdf" download> 이력서 다운로드 </a>
```

#### 3. iframe으로 외부 콘텐츠 삽입
웹페이지 내에서 외부 웹 페이지나 콘텐츠를 삽입하고 싶을 때 사용한다.
예를 들어, Youtube 동영상이나 다른 웹 페이지를 내 웹사이트에 보여줄 수 있다.
```
<iframe width="560" height="315" src="https://www.youtube.com/동영상ID" allowfullscreen></iframe>
```
